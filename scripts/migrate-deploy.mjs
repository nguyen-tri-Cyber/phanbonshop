...
console.log('================================================================');
console.log('STARTING DATABASE MIGRATION DEPLOY (ALL SERVICES)');
console.log('================================================================\n');

let failed = false;

for (const svc of services) {
  console.log(`▶ Deploying migrations for [${svc.name}]...`);
  try {
    const output = execSync(`npx prisma migrate deploy --schema=${svc.schema}`, {
      cwd: rootDir,
      env: process.env,
      encoding: 'utf8',
    ...
    console.log(`✔ [${svc.name}] Migration deploy successful!\n`);
  } catch (error) {
    console.error(`✖ [${svc.name}] Migration deploy FAILED!`);
...
if (failed) {
  ...
  console.log('================================================================');
  console.log('ALL SERVICE MIGRATIONS DEPLOYED SUCCESSFULLY (IDEMPOTENT)');
  console.log('================================================================');
}
