...
export class InventoryProxyController {
  ...
  @All(['internal/v1/inventory', 'internal/v1/inventory/*'])
  async handleInternalInventory(
    @Req() req: RequestWithId,
    @Res({ passthrough: true }) res: Response,
  ): Promise<unknown> {
    return this.forward(req, res);
  ...
  private async forward(req: RequestWithId, res: Response): Promise<unknown> {
    const targetUrl = `${this.inventoryServiceUrl}${req.originalUrl || req.url}`;
    ...
    try {
      ...
      logger.error(`Forwarding request to inventory-service failed: ${targetUrl}`, err);
      throw new HttpException(
        {
          code: 'BAD_GATEWAY',
          message: 'Không thể kết nối tới dịch vụ quản lý kho (Inventory Service)',
        },
